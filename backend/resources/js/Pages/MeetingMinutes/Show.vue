...
<template>
    <Head :title="`Notulensi: ${props.minute.agenda}`" />
    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                ...
                <Link 
                    :href="route('meeting-minutes.edit', props.minute.id)" 
            ...
            <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    <div class="flex items-start justify-between mb-4">
                        <div class="flex-1">
                            <h1 class="text-2xl font-bold text-gray-900 mb-2">{{ props.minute.agenda }}</h1>
                            <div class="flex items-center text-sm text-gray-600">
                                <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                </svg>
                                {{ formatDate(props.minute.meeting_date) }}
                            </div>
                    ...
                    <div class="flex items-center pt-4 border-t border-gray-200">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-medium text-gray-900">{{ props.minute.creator?.name || '-' }}</p>
                        </div>
                    </div>
                </div>

                <!-- Hasil Notulensi (Judul + Isi dalam satu section) -->
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    <div 
                        v-if="props.minute.notes" 
                        class="prose max-w-none text-gray-700"
                        v-html="props.minute.notes"
                    ></div>
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <span class="text-sm text-gray-500">{{ props.minute.attachments?.length || 0 }}</span>
                    </div>
                    
                    <div v-if="props.minute.attachments?.length > 0" class="space-y-2">
                        <a
                            v-for="att in props.minute.attachments" 
                            :key="att.id"
