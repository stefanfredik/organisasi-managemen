    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex justify-between items-center">
                ...
                <PrimaryButton @click="showModal = true" v-if="$page.props.auth.user.role === 'admin' || $page.props.auth.user.role === 'bendahara'">
                    Catat Transaksi
                </PrimaryButton>
            </div>
        </template>
                                ...
                                <tr v-for="finance in finances.data" :key="finance.id" class="hover:bg-gray-50 transition">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex justify-end gap-2">
                                            ...
                                            <button @click="deleteFinance(finance)" v-if="$page.props.auth.user.role === 'admin' || $page.props.auth.user.role === 'bendahara'" class="text-red-300 hover:text-red-500 transition p-2" title="Hapus">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
